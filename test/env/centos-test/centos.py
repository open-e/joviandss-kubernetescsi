...
test_name = "centos-test"

vfile = """
# -*- mode: ruby -*-
# vi: set ft=ruby :
Vagrant.configure("2") do |config|
  config.vm.box = "<centos-vm-name>"
  config.vm.synced_folder "../src", "/src", owner: "vagrant", group:"vagrant"
  config.vm.synced_folder "../configs", "/configs", owner: "vagrant", group:"vagrant"
...
def init_vm(name, root):
    ...
    v = vfile.replace("<centos-vm-name>", name)
...
def publish_container(root, argsi, version):
    ...
    if args.dpl == True:
        ...
        set_tag_latest = ("docker tag opene/joviandss-csi-c:" + version +
                            " opene/joviandss-csi-c:latest")
        con.sudo(set_tag_latest)

        upload_latest = "docker push opene/joviandss-csi-c:latest"
        con.sudo(upload_latest)

    if args.dpv == True:
        print(" - Publishing with tag " + version)
        upload_latest = "docker push opene/joviandss-csi-c:" + version
        con.sudo(upload_latest)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--test-vm', dest='tvm', type=str, default="generic/centos8",
